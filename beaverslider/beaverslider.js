function BeaverSlider(d) {
    ...
    this.constructAreaMain = function() {
        if (this.settings.type == "zoomer") {
            ...
            this.areaMain.css("background-size","100% 100%");
        }
    ...
    this.drawCells = function(n, m) {
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                ...
                }).append(jQuery("<div>").css(" background-size","100% 100%"))));
                h = (j == n - 1) ? 0 : (h - e)
            }
            r -= f
        }
    ...
    this.slideOverLeft = function(g) {
        ...
        f.css("background-size",this.settings.structure.container.width + "px " + this.settings.structure.container.height + "px");
        f.animate({
            left: 0,
            opacity: 1
        }, parseInt(this.currentEffect.duration), function() {
    ...
    var h = (this.settings.structure.container.height/2) - 65;
    $(".player-container-carousel").css({"width":this.settings.structure.container.width + "px ","margin-top":h + "px"});
    
    var closeBtn = $("<img id='closeBtn' src='images/close.png' />");
    closeBtn.css({"z-index":"10000","cursor":"pointer","position":"absolute","top":"0","right":"0","margin":"10px","display":"none"});
    closeBtn.click(
    		function(){
    			$("#my-slider").empty();
    			$("#sliderConteiner").hide();
    			slider = null;
    		});
    $("#my-slider").append(closeBtn).mouseover(function(){$("#closeBtn").show()}).mouseout(function(){$("#closeBtn").hide()});
